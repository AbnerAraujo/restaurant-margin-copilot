...
- The 2-year synthetic dataset (`backend/cmd/gendata`, `backend/data/live/`) is assumed present for meaningful year-over-year testing of User Story 2, but every requirement here also degrades correctly against the smaller 14-day fixture (`backend/fixtures/`) per FR-013 — this spec does not depend on any specific dataset being loaded.
