...
**Input**: User description: "Product D from the original 5-products comparison — Cross-Platform Economics Comparator (iFood vs. Just Eat Takeaway). Originally scored as not buildable by the take-home deadline because it needed 'two internally-consistent platform data models' — but both platforms already exist in the real ingested data with genuinely different commission rates (iFood 23% flat, Just Eat Takeaway 20% flat, per backend/cmd/gendata/opening/README.md), so this is now a tractable incremental feature of the existing product, not a separate product requiring new data."

## Background

`docs/product-strategy.md`'s 5-products comparison scored this concept highest on strategic differentiation ("most novel, Prosus-unique — something only Prosus, owning both platforms, could build in good faith") but lowest on near-term feasibility, specifically because it appeared to need two separately-modeled platform economics that didn't yet exist. They already exist: `delivery_platform_export.csv` carries a real `commission_rate_pct` per order, and the two platforms in this dataset carry genuinely different flat rates (iFood 23%, Just Eat Takeaway 20%). This feature surfaces that real difference as a first-class comparison, rather than leaving it implicit inside a single blended margin figure.
...
**Why this priority**: This is the whole point of the feature and is fully deliverable from data already ingested — no new data engineering, no new ingestion pipeline.

**Independent Test**: Using the dataset's hand-authored 14-day opening window, verify the comparator reports iFood's and Just Eat Takeaway's actual gross sales, actual commission paid, and actual effective commission rate, each independently checkable against `backend/cmd/gendata/opening/README.md`'s own reference tables.
...
**Independent Test**: Verify a platform's combined commission-plus-promo-spend total, and its net effective take rate, against an independent hand-computation from the same source CSVs.
...
- What happens when a period includes a day with a missing delivery-platform source (the dataset's known "missing day" case)? That day's contribution to both platforms' totals must be honestly absent, not zero-filled, consistent with how the existing reconciliation already handles this case (a discrepancy flag, not a silent zero).
...
- What happens when commission rates change over time (a real-world possibility this dataset does not model, since both rates are flat for the whole period)? Out of scope for this spec — see Assumptions.
...
- **SC-001**: Every figure in the comparator for the dataset's hand-authored 14-day opening window matches an independent hand-computation from the same source CSVs, to the cent.
...
- This feature does not require new ingestion or data engineering — it is a new read/computation over data this product already has.
