...
**A meta-question about the product itself is not a refusal case — catch it
before the classifier does.** "What do you do?", "how can you help me?",
"what can I ask?" are real, common inputs, and they are answerable by
definition: the product knows its own fixed capability list better than any
classifier could infer it. Routing this through the same
answerable/ambiguous/unanswerable gate as a data question produces the worst
outcome available — an honest classifier correctly sees no data-grounded
question at all and refuses, which reads to the user as "the product doesn't
even understand its own job." Pattern-match a small, fixed set of real
phrasings for this *before* the classification call runs at all, and answer
directly from the same hand-written capability list Move 5 already puts on
the empty state — never generated by asking a model to describe itself
(the model will happily invent a capability that doesn't exist, which is
the same class of lie a fabricated data answer would be). This costs zero
tokens and zero latency, and it turns the single most common "I don't know
what to ask" moment into the product's best first impression instead of its
first refusal.
...
- [ ] Is a meta-question about the product itself ("what do you do?", "how
      can you help?") caught and answered directly, before the classifier
      ever sees it — rather than correctly-but-uselessly refused as
      out-of-scope?
...
| Letting a meta-question ("what can you do?") reach the same classifier as a data question | Correctly judged out-of-scope and refused — technically honest, but reads as the product not understanding its own job on the single most common "I don't know what to ask" moment. |
...
- **Move 5, meta-question case:** a small set of real phrasings ("what do
  you do?", "how can you help me?", "what can I ask?") is pattern-matched
  *before* the classification call, and answered directly from the same
  hand-written capability list Move 5 already shows on the empty state.
  Before this existed, these questions reached the classifier, were
  correctly judged out-of-scope for the underlying data, and were refused —
  technically honest, but the worst possible first impression for a user who
  opened the product not knowing what to ask. Gated on there being no
  clarification already in flight, so a bare reply to an unrelated
  clarifying question is never mistaken for a fresh meta-question just
  because its text happens to match.
