...
/**
 * A short, plain-language, grouped summary of one day's flags — never each
 * flag's raw `detail` sentence concatenated. Reported live: a POS-heavy sync
 * day can legitimately carry 40+ `cross_source_duplicate_removed` flags
 * alone, and joining every one's technical detail (internal type names,
 * `simulated://...` provenance URIs, row numbers) produced a wall of jargon
 * in a badge meant to carry one line of context (see
 * `ReconciliationBadge.detail`'s own doc comment in BadgeDisplay.tsx).
 *
 * Grouped by type and counted, ordered by what the OWNER needs to act on —
 * an overlap the system could not resolve outranks an amount difference,
 * which outranks a duplicate that was already caught and handled — then
 * capped to the two most important groups, with the rest folded into a
 * plain "N more things flagged" rather than dropped or spelled out in full.
 */
function summarizeFlags(flags: DiscrepancyFlagApi[]): string {
  ...
  return [
    {
      ...
      date: day.date,
      detail: isClean ? undefined : summarizeFlags(day.discrepancy_flags),
    },
  ]
}
