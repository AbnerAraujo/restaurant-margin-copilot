...
- [x] **Hand-authored test data** (T008) — authored: a two-week window, four source files, every deliberate irregularity documented with exact row IDs and independently-verified reference sums (today: the dataset's opening window, `backend/cmd/gendata/opening/README.md`).
...
- **007 — Cost sheet upload**: ✅ Ready. Spec and plan written (`specs/007-cost-sheet-upload/`); reuses `internal/ingest.ParseCostSheet` and `internal/pipeline.RunIngestionPipeline` unchanged, so no new validation logic needed reviewing — the only new risk (accidentally writing outside the dataset directory) is closed by construction via a compile-time-anchored data path (`internal/livedata`), not a runtime check, per the plan's own Constitution Check.
- **009 — Business Insight Advisor**: ✅ Ready. Spec, plan, and tasks written (`specs/009-business-insight-advisor/`), with the one categorical risk this feature introduces — probabilistic advice being mistaken for a computed fact — closed by construction rather than by convention: the trigger and title are deterministic Go (zero model calls), the billed advice call is strictly opt-in and re-verified server-side against the posted tool results, its output is visually and structurally segregated with an on-the-wire disclaimer, and every call lands in its own dedicated `business_insight_interaction` ledger. The research dependency (real commission-tier, dispute-window, and cost-cadence sources for the prompts and thresholds) was resolved during planning, with each claim tagged Sourced vs. Judgment in plan.md and one unverifiable statistic explicitly excluded rather than cited.
