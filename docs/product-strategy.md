...
**Chart click-to-ask needed a bigger mechanism than tasks.md assumed (User Story 1).** T012 assumed a shared hook might already expose `ChatPanel`'s `submitQuestion` across pages. It doesn't — `/close`, `/promotions`, and `/ask` are fully separate routes with no shared chat context, and nothing can call a function on a component that isn't mounted. The real fix: a chart click calls `navigate('/ask', {state: {autoSubmitQuestion}})`, and `ChatPanel` gained a ref-guarded effect that submits `autoSubmitQuestion` exactly once per distinct value on mount. This is a small but real architectural addition — "click a chart, land in chat with the question already asked" now works across a full page navigation, not just within one page's local state.

**The flag-based follow-up needed to compete for a slot, not just exist (User Story 1).** `deriveFollowUpSuggestions` was, before this change, a single-source function — only one tool's follow-up template ever populated the candidate list at all. Simply adding a new candidate source risked it never actually appearing if the primary source's template already filled all 3 slots. The implementation prepends the flag-based candidate instead of appending it, so a real discrepancy always wins a slot rather than being silently crowded out — a small ordering choice with a real behavioral consequence.

**"Show your work" and "resolved period" both turned out to need genuinely new `AskResponse` fields (User Stories 1 and 2), not existing data reused.** Both were open questions in plan.md, both required actually reading `ask.go`/`explain.go` rather than assuming: `ToolCalls []ToolCallView` (US1, tool name + raw JSON, `omitempty`) and `resolved_period` (US2, scoped to only the two tools with a top-level date field, `get_period_totals`/`get_daily_summary`) are both new, additive fields, landed by two different agents in the same file without conflict because each read the other's already-committed state first.

**The Platforms effective-rate trend got a new backend endpoint, not a frontend-assembled one (User Story 2).** Plan.md's own stated threshold — prefer assembling from existing endpoints unless it would take more than 2-3 sequential calls — was applied honestly rather than reflexively avoiding backend work: a trailing 6-month trend needs 6 calls, which crossed the stated line, so `GET /api/platforms/trend` is a real new deterministic (no model call) endpoint wrapping `compare_platform_economics` across a bounded window, skipping (never fabricating) any period the tool refuses as `insufficient_data`.

**FR-005's out-of-range refusal was verified live, not just unit-tested (User Story 2).** Rather than trusting that the existing ambiguity gate's out-of-range wording would read sensibly for a comparison-flavored question (the exact underspecification the analysis pass flagged and T020 was tightened to require a definite check for), the implementing agent ran a real out-of-range comparison question against the actual running dev backend and confirmed the refusal names the specific out-of-range date and states the real data range — the tightened acceptance check passed on the first real attempt, with no new refusal path needed.

**The Home trend arrow's "yesterday" is the previous data point, not calendar-yesterday (User Story 3).** This dataset has a real, deliberate ingestion gap (documented since the original fixture design). A fixed calendar-day offset would silently produce no comparison across that gap even though a perfectly good prior day exists one slot back in the data. "Yesterday" here means "the immediately preceding entry in the days array" — a small choice, but the only one that doesn't quietly under-serve the exact kind of messy-real-world day this project was built to handle honestly.
...
## 2026-08-29 — Real months, not just real days: monthly loss regimes in the synthetic live dataset

The previous fix (`5dd0c7b`, this same day) added a 6%/day independent cost-shock chance and verified 65/744 days (8.7%) came out net-negative. Live usage since then surfaced the deeper miss: **almost no whole MONTH in the 24-month synthetic history was a net loss.** A day or two of bad margin gets absorbed by ~28 good days every time, so the dataset read as "this restaurant basically always wins," not as a real small business with real bad stretches — even though the user's own target for the business (**$40,000/month average** in this product's margin metric, their own stated figure, not researched) was never meant to imply zero-volatility perfection.

**Research, tagged per this doc's Sourced/Assumption/Hypothesis convention (line 5):**

- **[Sourced]** Independent-restaurant net margins commonly run 3-9% of revenue, skewing toward the low end for full-service independents ([VantaInsights' 2026 restaurant-benchmarks synthesis](https://vantainsights.com/insights/restaurant-profit-margins), drawing on Census CBP/BLS/Fed/SEC-informed industry data — an aggregated secondary analysis, not one primary study). [Toast/VantaInsights](https://pos.toasttab.com/blog/on-the-line/average-restaurant-profit-margin) puts only ~42% of U.S. restaurants profitable in 2024; [Restaurant365's 2026 State of the Restaurant Industry report](https://www.restaurant365.com/guides/the-restaurant-profitability-playbook-for-2026/) puts **45% of operators unprofitable in 2025** (methodology not disclosed beyond "survey"). A large minority of the real industry runs at an annual loss — "almost never a bad month" was never a realistic reading of that backdrop, even for a restaurant modeled as one of the healthier operators overall.
- **[Sourced]** [The NPD Group's Seasonality Index for Total Restaurant Traffic](https://www.nrn.com/restaurant-finance/4-ways-restaurants-can-fend-off-a-january-sales-slump): January traffic runs ~6% below the average month and ~11% below the June peak, 2013-2019. Individual single-location restaurants are reported in trade coverage to see steeper slow-period drops, "as much as 30%" — a softer, less rigorous source, cited only to bound a plausible range.
- **[Sourced]** [JPMorgan Chase Institute](https://www.jpmorganchase.com/institute/all-topics/business-growth-and-entrepreneurship/report-growth-vitality-cash-flows)'s small-business cash-flow research finds restaurant-sector **expenses are more volatile than revenues** — the opposite pattern from most other small-business sectors. This directly shaped the mechanism: because every cost this product's margin metric counts (COGS/commission/refunds — no rent or labor, see `cmd/gendata/main.go`'s package doc) scales proportionally with revenue, **a demand dip alone cannot mathematically flip a month's margin negative** in this model. It takes a cost-side shock on top of it — exactly what JPMorgan's finding would predict, and exactly why the new January regime pairs a demand dip with an elevated run of cost-shock days rather than relying on the dip by itself.
- **[Sourced]** Wholesale egg prices ran ~70% above year-earlier levels in January 2023 ([CPI data via FoodNavigator-USA](https://www.foodnavigator-usa.com/Article/2024/01/11/egg-prices-jump-as-avian-flu-spreads-pushing-up-overall-food-prices-cpi-reveals/)), driven by avian flu — used only as a real-world scale/shape reference for how severe a genuine "supplier shortage" cost event can get, not a literal re-creation of the egg market.
- **[Assumption]** Which five specific months carry a regime, how many days within each get forced into an oversized cost shock, and the exact demand-dip/refund-rate multipliers are this project's own reasoned judgment — no source quantifies "restaurant-months net-loss frequency" or "January cost-shock-day frequency" precisely enough to cite directly. Tuned so a real, named cause drives every regime month (never an unexplained random dip), the healthy majority of months still carry the $40k/month growth story, and the regime months actually land net-negative once generated — checked empirically, not just assumed from the inputs.
- **[Assumption]** The $40,000/month target itself is the user's own stated figure for this product's specific margin metric (gross minus commissions minus refunds minus input costs — never full net profit after labor/rent, which this product has never modeled). It is not read as "3-9% net margin on revenue": at that ratio $40k/month would imply $450k-$1.3M/month of revenue, absurd for one location. The research above grounds the *shape* of the dataset's volatility, not this dollar figure.

**The model, concretely (`backend/cmd/gendata/main.go`):**

- Daily layer unchanged: `lossyDayChance` = 6%/day, cost shock sized 0.75-1.40x that day's own gross — the existing mechanic from `5dd0c7b`, left as one layer of noise on top of the new monthly structure.
- New monthly layer: 5 of the 24 months carry a `monthRegime`, each pinned to one coherent, named real-world cause and a **deterministic count** of that month's days forced into an oversized shock (a fixed count, not a per-day dice roll, so the regime's aggregate effect doesn't depend on how a probability happened to land):
  - **2025-01, 2026-01 — "January seasonal slump + cold-snap equipment strain":** 20% demand dip + 21 of that month's days forced into a cost shock, cause pinned to the existing "equipment" `costShockCauses` entries (walk-in cooler / plumbing failure — cold-weather equipment strain is a real, commonly cited restaurant-ops pattern, though not independently sourced here with a number).
  - **2025-04, 2026-04 — "Regional protein/produce shortage — emergency re-sourcing":** no demand change (per the JPMorgan finding above — a pure cost-side event), 19 days forced into a cost shock, cause pinned to the protein or produce supplier depending on year.
  - **2025-10 — "Food-safety complaint wave":** refund rate on delivery orders raised 18x (2% → 36%) with a distinct refund note ("Batch of undercooked orders reported..."), plus 17 forced shock days — modeling "a cluster of refunds/discrepancies" as its own distinct cause per the task brief, not folded into a cost-shock story.
- Growth curve raised from the previous $14,000→$33,500/month range (which targeted $20k/month margin) to $31,700→$115,000/month, empirically tuned — by actually running the generator and reading its own new monthly-verification printout, not solved for algebraically — so the 24-month average lands near $40k/month once the five regime deficits are averaged in.
- A real bug was caught and fixed along the way: the first version of `forcedShockDays` ranged over the regimes `map[string]monthRegime` directly, and Go randomizes map iteration order per run, so each month's `rng.Perm` call consumed a different slice of the shared seeded RNG stream depending on that run's arbitrary order — silently breaking this file's own long-documented "same seed, same dataset, every regen" guarantee. Caught by literally running `gendata` twice and diffing the output; fixed by sorting the month keys before iterating.

**Verified live, after regenerating `backend/data/live/` from scratch and re-ingesting into the real local Postgres database** (`go run ./cmd/server -ingest data/live -ingest-promo data/live`), then queried both directly against `daily_reconciliation` and through the running app's own `GET /api/reconciliation` endpoint (not just the generator's self-reported estimate):
- **5 of 24 months net-negative** (2025-01: -$9,685; 2025-04: -$10,596; 2025-10: -$4,574; 2026-01: -$6,816; 2026-04: -$20,840), each carrying its own coherent, dated `supplier_cost_sheet`/`delivery_platform` narrative (e.g. 2025-04's `daily_reconciliation` rows show repeated "Regional shortage price spike; emergency restock to cover service" invoices from the same protein supplier spread across the month, not one unexplained spike).
- **24-month average margin: $40,333/month** — squarely on the user's stated $40k/month target, computed by Postgres directly (`sum(margin)/24` over `daily_reconciliation`), not asserted from the generator's inputs.
- **142 of 730 days (19.5%) net-negative daily**, up from the prior 8.7% baseline — expected, since the regime months alone force 17-21 days each into an oversized shock; the increase is concentrated in the 5 regime months, not spread as new noise across the healthy months.
- Spot-checked: 2025-11 (a healthy month, no regime) totals **+$77,207** through the live API, in line with its place on the growth curve; 2025-04's loss story checks out end-to-end from cause (`supplier_cost_sheet`) through effect (`daily_reconciliation.margin`) via the actually-running backend, not just the offline CSVs.
- `cd backend && go build ./... && go vet ./... && go test ./...` all pass, including the live-Postgres integration tests (`DATABASE_URL` set) that read the freshly reloaded dataset — `TestHandlePlatformComparison_DefaultsToRealDataRange` derives its expectation from `storage.LoadDataDateRange` at runtime, so it tolerated the new data without modification, exactly as intended when it was split out in the entry above this one.

### Same-day addendum: August itself had been skipped, plus a second real bug found while fixing it

A live check of the numbers above by the user surfaced a real gap the five-regime version missed: **August specifically never got a regime, in either historical year.** That matters more than an arbitrary missed month — `backend/data/live/` covers 2024-08-01 through 2026-07-31, and 2026-08-01 onward is the separate, protected `backend/fixtures/` 14-day eval set. **2025-08 is therefore the only full historical August in the live dataset**, making it exactly the comparator any "this August vs. last August" chat question would use — and it was landing as a clean, all-profit month, undercutting the point of this whole fix for the single month someone actually asking that question right now would hit.

**New research, same tagging convention:** August sits in the middle of this dataset's summer upswing (a strong month on both sides), so a demand-side "slow season" story doesn't fit the way January's does — the cause needed to be cost-side, consistent with the JPMorgan expense-volatility finding already used above. **[Sourced, trade-press tier]** Several commercial-refrigeration/HVAC service sources ([Culinary Depot](https://www.culinarydepotinc.com/blog/maintaining-a-commercial-refrigeration-unit-during-the-summer/), [B&J Refrigeration](https://bjrefrigeration.com/why-walk-in-coolers-fail-in-extreme-heat-and-how-tucson-businesses-can-prevent-costly-breakdowns/), [RepairPros](https://www.repairprosservices.com/post/why-extreme-heat-is-causing-more-hvac-and-walk-in-cooler-failures), [rentcoolcubes.com](https://rentcoolcubes.com/blogs/cool-cubes-blog/summer-heat-wave-emergency-cold-storage)) converge on the same mechanism: summer heat pushes walk-in cooler compressors to run continuously instead of cycling, and dirty condenser coils plus AC-driven voltage sag on the hottest afternoons make mid-summer the highest-failure-rate season for commercial refrigeration — a real, well-covered restaurant-ops topic, but contractor-blog-tier sourcing, not a formal industry report, hence the lower confidence tag than the NPD/JPMorgan/CPI citations above. **[Sourced, authoritative]** The FDA's food-safety "danger zone" rule adds the reason a summer failure turns into discarded inventory faster than the same failure in a cooler month: perishables held 40-140°F become unsafe within 2 hours generally, within 1 hour above 90°F ambient.

**The fix:** a new `regimeHeatWave` kind, assigned to `"2025-08"` (only — there is no 2026-08 in this dataset), demand unaffected (pure cost-side event, matching `supplierShortage`'s pattern), 21 of August's 31 days forced into an oversized cost shock, pinned to a new dedicated `costShockCauses` entry ("Heat-wave compressor overload — walk-in ran warm for hours; spoiled stock discarded and re-bought same-day at rush pricing"). Adding a 6th loss month pulled the 24-month average down (to ~$36.9k/month at the prior growth-curve constants), so `startMonthlyGross`/`endMonthlyGross` were re-tuned upward (from $31,700→$115,000 to **$34,400→$124,700**) rather than left to drift, restoring the ~$40k/month target — the same empirical-tuning-against-the-real-printout approach as the original fix, not solved algebraically.

**A second real bug, caught before shipping this addendum:** the new heat-wave cause was initially added directly into the shared `costShockCauses` slice that an ORDINARY (non-regime) `lossyDayChance` day picks from at random — which meant a plain December or April day could randomly draw the "heat wave" narrative too, discovered by reading the regenerated `supplier_cost_sheet.csv` and finding a heat-wave invoice dated 2024-12-22. Fixed by splitting a `generalCostShockCauses` pool (the original four causes) for ordinary random days, leaving season-pinned causes like the heat-wave entry reachable only through a forced regime day's own `causeIdx`. This changes only which supplier/note text an ordinary shock day gets, never any dollar amount — confirmed by an unchanged monthly-margin printout before and after the fix.

**Re-verified live**, same method as above (regenerate `data/live/`, re-ingest both pipelines, query `daily_reconciliation` directly and through `GET /api/reconciliation`):
- **6 of 24 months net-negative**: 2025-01 (-$6,447), 2025-04 (-$13,977), **2025-08 (-$5,750)**, 2025-10 (-$13,232), 2026-01 (-$10,331), 2026-04 (-$15,612).
- **24-month average margin: $40,428/month.**
